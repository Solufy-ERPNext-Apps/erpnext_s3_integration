...
def generate_s3_key(file_doc, settings):
    ...
    filename = unidecode(
        file_doc.file_name or file_doc.file_url.split("/")[-1]
    ).replace(" ", "_")

    # Include hash if available, else just filename
    identifier = (
        f"{file_doc.content_hash}-{filename}" if file_doc.content_hash else filename
    )
...
def before_insert(file_doc, method):
    ...
    if (
        hasattr(file_doc, "is_file_path")
        and file_doc.is_file_path()
        and not frappe.flags.in_test
    ) or getattr(file_doc, "is_folder", False):
        print(f"Is file path or is folder")
        return
    ...
    try:
        ...
        content_stream = (
            ...
            else io.BytesIO(content.encode())
        )
        s3_client.upload_fileobj(
            content_stream, s3_key, file_doc.get("mime_type"), is_public
        )
