...
def after_backup():
    ...
    for file in files:
        if file.startswith(today_str):
            ...
            if (is_db and not settings.upload_db_backup) or (
                is_files and not settings.upload_files_backup
            ):
                continue
            ...
            s3_key = (
                f"{folder_prefix}{backup_prefix}{site_name}/{year}/{month}/{day}/{file}"
            )
...
def log_s3_sync(status, message):
    try:
        ...
        frappe.log_error(f"Failed to create S3 Sync Log: {str(e)}", "S3 Sync Log Error")
...
def scheduled_backup_and_sync():
    ...
    try:
        ...
        log_s3_sync(
            "Failed",
            f"Scheduled S3 Backup Failed: {str(e)}\n\n{frappe.get_traceback()}",
        )
