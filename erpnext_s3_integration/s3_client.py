...
class S3Client:
    ...
    def get_password(self, fieldname):
        ...
        try:
            return get_decrypted_password(
                "S3 Integration Settings", "S3 Integration Settings", fieldname
            )
    ...
    def setup_client(self):
        ...
        if not (aws_access_key_id and aws_secret_access_key):
            frappe.throw("AWS Credentials are required to initialize the S3 client.")

        self.bucket_name = self.settings.bucket_name
        if not self.bucket_name:
            frappe.throw("AWS Bucket Name is required.")
    ...
    def test_connection(self):
        try:
            ...
            frappe.log_error(
                message=frappe.get_traceback(), title="S3 Test Connection Error"
            )
            return False, f"Connection Failed: {e}"
        except Exception as e:
            frappe.log_error(
                message=frappe.get_traceback(), title="S3 Test Connection Failed"
            )
    ...
    def upload_fileobj(self, fileobj, key, content_type=None, is_public=False):
        ...
        try:
            ...
            frappe.log_error(
                message=frappe.get_traceback(), title=f"S3 Upload Failed for {key}"
            )
    ...
    def delete_object(self, key):
        try:
            ...
            frappe.log_error(
                message=frappe.get_traceback(), title=f"S3 Delete Failed for {key}"
            )
    ...
    def download_as_stream(self, key):
        try:
            ...
            frappe.log_error(
                message=frappe.get_traceback(), title=f"S3 Downoad Failed for {key}"
            )
